...
public class SM2Utils {
	...
	*/
    ...
    public static byte[] sign(byte[] userId, byte[] privateKey, byte[] sourceData) throws IOException {
        ...
        v2.add(d_r);
        v2.add(d_s);
        DERSequence ds = new DERSequence(v2);

        /**
         * TODO used to verify if the output is as expected
         */
//        DERObject sign = ds;
//        byte[] signdata = sign.getDEREncoded();
        byte[] signdata = ds.getEncoded();
        return signdata;
    }
    ...
    public static boolean verifySign(byte[] userId, byte[] publicKey, byte[] sourceData, byte[] signData) throws IOException {
        ...
        ASN1InputStream dis = new ASN1InputStream(bis);
        ASN1Primitive a1p = dis.readObject();
        ASN1Sequence a1s = ASN1Sequence.getInstance(a1p);
        Enumeration<ASN1Integer> e = a1s.getObjects();
//        DERObject derObj = dis.readObject();
